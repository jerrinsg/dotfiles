...
function promptcmd () {
    ...
    if [ ${STPJBS} -gt 2 ]; then
        ...
        PS1="${PS1}\[${COLOR_BROWN}\][stp:${STPJBS}] "
    fi
    ...
    # Working directory
    if [ -w "${PWD}" ]; then
        PS1="${PS1}\[${COLOR_BROWN}\]$(prompt_workingdir)"
    else
...
function load_prompt () {
    # Get PIDs
	parent_process_file="/proc/$PPID/cmdline"
	my_process_file="/proc/$$/cmdline"

	local parent_process=""
	local my_process=""

	if [ -f $parent_process_file ];
	then
		local parent_process=$(cat /proc/$PPID/cmdline | cut -d \. -f 1)
	fi

	if [ -f $my_process_file ];
	then
